...
起動・発注・完了検知・報告回収が期待どおりに動かないときに読む。症状を見て運用を想像で変える前に、実際を確認する。通常の運用は `execution.md`、ランナーごとのコマンドは `runners.md`。
...
- ランナーがその引数を渡せない。通らない設定と代替は `runners.md`。
- 報告を画面から読むランナーで、Codex をインライン描画で起動していない。回収で詰まるので、この時点で畳んで起動し直す（`runners.md`）。
...
まずランナー側の制約を疑う。画面から読むランナーで Codex を代替画面のまま起動していると、出力がスクロールバックへ入らず、行数を増やしても報告の先頭は戻らない（`runners.md`）。0行が返るのは失敗ではなく「まだ流れていない」状態のこともある。この状態で先頭が要るなら、報告をMarkdownでファイルへ書いてパスだけを返すよう依頼し直して読む。最初の依頼文でファイル出力を求めない。
...
- 引数を渡せないランナーでは、既存のプロバイダセッションを取り込む経路を使う（Paseo は `paseo import <セッションID> --provider codex --cwd <作業ツリー>`。`runners.md`）。
