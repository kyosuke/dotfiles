...
新しい依頼は新しいエージェントで出す。別の名前でペインを立て、`agent start` からやり直す。依頼文は単体で完結する形に整えてあるので（`delegation-template.md`）、前のスレッドを引き継いで得るものがない。引き継がれるのは前の依頼の調査結果、却下した案、検収を終えた差分の記憶で、Codex はそれを現在の前提として扱う。範囲の外へ手が伸びたり、依頼文に書いた事実より古い理解で動いたりする余地が増える。

同じスレッドへ続けるのは、それまでのやり取りが作業の前提になる場合に限る。
...
前のエージェントは `/quit` で畳んでから次を立てる（下記「スレッドIDと後片付け」）。畳まずに増やすと、ペインが埋まるうえに、同一作業ツリーで別の Codex が走る事故も起きる（上記「待機中の判断」）。あとで文脈が要ると分かったときは `agent_session.value` から再開できるので、UUID は控えておく。
