...
運用の前提そのものを変えるときに読む。通常の発注・検収では読まなくてよい。同じ検討を繰り返さないための記録なので、結論だけを運用側（`../SKILL.md`・`ordering.md`・`execution.md`・`review.md`・ランナーの reference）へ置き、根拠はここへ残す。

## ユーザーからの指示の履歴

運用側の規則は日付を持たずに書く。どの規則がいつ、どの観測を受けて決まったかはここで引く。
...
2026-07-24 から 2026-09-17 まで、タスクごとに `~/.claude/codex-director/trial-log.md` へ記録を付けた（53タスク、約110KB）。項目はタスク概要 / プロファイル / 一度で完了したか / 修正依頼の回数 / 合否 / プロファイル選択の適否 / 1段下げたか / 権限の昇格。プロファイル体系を2回組み替え（`sol`/`terra`/`luna` の3段 → `luna` 既定 → Paseo のプロファイル4つ）、選択の根拠はこのファイルの「プロファイルの選択」へ昇格済みである。

2026-09-18 に終了した。理由は、ログが次のタスクで読まれる保証がなく、再発する制約は `references/` へ書くほうが効くからである。ファイルは履歴として残し、追記はしない。
